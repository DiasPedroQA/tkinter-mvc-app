...
#         self.janela: tk.Tk = janela
#         self.janela.title(string="FileMaster Pro")
#         self.janela.geometry(newGeometry="800x750")
#         self.janela.minsize(width=600, height=600)
...
#         self.style = ThemedStyle(master=self.janela)
#         self.style.set_theme(theme_name="equilux")  # Tema escuro moderno
#         self.style.configure(style="TButton", padding=6)
#         self.style.configure(style="TCombobox", padding=5)
...
#         cabecalho = ttk.Frame(master=self.janela, padding="10 5")
...
#         painel = ttk.Label(master=self.janela, text="Controle", padding=10)
#         painel.pack(fill=tk.X, padx=10, pady=5)

#         # Frame para caminho de origem
#         elemento_caminho_origem = ttk.Frame(master=painel)
#         elemento_caminho_origem.pack(fill=tk.X, pady=5)

#         ttk.Label(master=elemento_caminho_origem, text="Caminho de Origem:").pack(
#             side=tk.LEFT
#         )
#         self.entrada_origem = ttk.Entry(master=elemento_caminho_origem)
#         self.entrada_origem.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

#         self.combo_extensao_origem = ttk.Combobox(
#             master=elemento_caminho_origem,
#             values=[".txt", ".csv", ".json", ".html", ".pdf"],
#             width=7,
#             state="readonly",
#         )
#         self.combo_extensao_origem.set(value=".txt")
...
#         elemento_caminho_destino = ttk.Frame(master=painel)
#         elemento_caminho_destino.pack(fill=tk.X, pady=5)

#         ttk.Label(master=elemento_caminho_destino, text="Caminho de Destino:").pack(
#             side=tk.LEFT
#         )
#         self.entrada_destino = ttk.Entry(master=elemento_caminho_destino)
#         self.entrada_destino.pack(side=tk.LEFT, expand=True, fill=tk.X, padx=5)

#         self.combo_extensao_destino = ttk.Combobox(
#             master=elemento_caminho_destino,
#             values=[".txt", ".csv", ".json", ".html", ".pdf"],
#             width=7,
#             state="readonly",
#         )
...
#         painel = ttk.Frame(master=self.janela)
#         painel.pack(fill=tk.BOTH, expand=True, padx=10, pady=5)

#         # Notebook (abas)
#         self.notebook = ttk.Notebook(master=painel)
#         self.notebook.pack(fill=tk.BOTH, expand=True)

#         # Aba de resultados
#         self.aba_resultados = ttk.Frame(master=self.notebook)
#         self.notebook.add(self.aba_resultados, text="Resultados")

#         self.treeview = ttk.Treeview(
#             master=self.aba_resultados,
#             columns=("tipo", "tamanho", "modificacao"),
#             show="headings",
#             selectmode="extended",
#         )
#         self.treeview.heading(column="tipo", text="Tipo")
#         self.treeview.heading(column="tamanho", text="Tamanho")
#         self.treeview.heading(column="modificacao", text="Modificação")
#         self.treeview.column(column="tipo", width=100)
#         self.treeview.column(column="tamanho", width=100)
#         self.treeview.column(column="modificacao", width=150)

#         scroll_y = ttk.Scrollbar(
#             master=self.aba_resultados, orient=tk.VERTICAL, command=self.treeview.yview
#         )
...
#         self.aba_resultados.grid_rowconfigure(index=0, weight=1)
#         self.aba_resultados.grid_columnconfigure(index=0, weight=1)

#         # Aba de logs
#         self.aba_logs = ttk.Frame(master=self.notebook)
#         self.notebook.add(child=self.aba_logs, text="Logs")

#         self.log_text = scrolledtext.ScrolledText(
#             master=self.aba_logs, wrap=tk.WORD, font=("Consolas", 10)
#         )
#         self.log_text.pack(fill=tk.BOTH, expand=True)

#         ttk.Button(
#             master=self.aba_logs, text="Limpar Logs", command=self._limpar_logs
#         ).pack(pady=5)

#     def _criar_rodape(self) -> None:
#         """Cria o rodapé da aplicação"""
#         rodape = ttk.Frame(master=self.janela, padding="5")
#         rodape.pack(fill=tk.X)

#         ttk.Label(
#             master=rodape,
#             text=f"Base do usuário: {self.backend.base_usuario}",
#             font=("Segoe UI", 8),
#         ).pack(side=tk.LEFT)

#         ttk.Label(
#             master=rodape, text="v1.0 © 2023 FileMaster Pro", font=("Segoe UI", 8)
#         ).pack(side=tk.RIGHT)
...
#         caminho: str = self.entrada_origem.get()
#         extensao: str = self.combo_extensao_origem.get().lstrip(".")

#         if not caminho:
#             messagebox.showwarning(
#                 title="Aviso", message="Por favor, informe um caminho para análise"
#             )
#             return

#         self._log(mensagem=f"Analisando caminho: {caminho}")

#         try:
#             self.resultado_analise = self.backend.server_analisar_caminhos(
#                 caminho, extensao
#             )
#             self._exibir_resultados()
#         except Exception as e:
#             self._log(mensagem=f"Erro na análise: {str(e)}", erro=True)
#             messagebox.showerror(
#                 title="Erro", message=f"Falha ao analisar caminho:\n{str(e)}"
#             )
...
#                     parent="", index=tk.END, values=(arquivo, "Pasta", "")
#                 )
...
#             messagebox.showwarning(title="Aviso", message="Analise um caminho primeiro")
#             return

#         extensao: str = self.combo_extensao_destino.get().lstrip(".")

#         try:
#             self._log(mensagem=f"Iniciando conversão para {extensao}...")
#             self.backend.server_converter_arquivos(
#                 self.arquivos_selecionados or [self.resultado_analise.caminho_tratado],
#                 extensao,
#             )
#             self._log(mensagem="Conversão concluída com sucesso!")
#         except Exception as e:
#             self._log(mensagem=f"Erro na conversão: {str(e)}", erro=True)
#             messagebox.showerror(title="Erro", message=f"Falha na conversão:\n{str(e)}")
...
#             self._log(mensagem=f"Arquivo selecionado: {valores[0]}")
...
#         self.log_text.configure(state="normal")
#         self.log_text.delete(index1=1.0, index2=tk.END)
#         self.log_text.configure(state="disabled")


# if __name__ == "__main__":
#     root = tk.Tk()
#     app = FileMasterApp(janela=root)
...
#         self.master = master
#         self.master.title("Tkinter MVC App")
#         self.master.geometry("400x300")

#         self.frame = Frame(self.master)
#         self.frame.pack(pady=20)

#         self.label = Label(self.frame, text="Welcome to the Tkinter MVC App!")
#         self.label.pack()
